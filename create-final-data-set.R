...
mobility <- 
here::here("data", "2020_US_Region_Mobility_Report.csv") %>%
  read_csv() %>%
  select(state = iso_3166_2_code, date, contains("percent_change")) %>%
  filter(!is.na(state)) %>%
  ...
  mutate(
    ...
    workplaces = zoo::rollmean(
      workplaces_percent_change_from_baseline, 
      7, 
      NA, 
      "right"
    ),
    residential = zoo::rollmean(
  ...
  ungroup() %>% 
  select(-contains("percent_change_from"))
...
ltc <- 
here::here("data", "ltc_pop.csv") %>% 
  read_csv() %>% 
  rename(State = state) %>% 
  mutate(
    state = state.abb[match(State, state.name)],
    state = ifelse(State == "DC", "DC", state),
...
beds_staffed <-
  here::here("data", "beds.csv") %>%
  read_csv() %>%
  mutate(
    state = state.abb[match(Location, state.name)],
    state = ifelse(Location == "Washington D.C.", "DC", state),
    .before = Location
  ) %>%
  select(-Location)
